...
export default function PramuniagaPage() {
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-white font-sans flex flex-col selection:bg-violet-500/30 selection:text-white">
      {/* Top Navbar */}
      <nav className="border-b border-slate-800 px-6 py-4 flex justify-between items-center bg-[#0F172A] shadow-md sticky top-0 z-30">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-violet-400 font-semibold uppercase tracking-wider">Modul Pramuniaga</p>
          </div>
        </div>
        
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2.5 bg-slate-800/40 border border-slate-850 px-4 py-1.5 rounded-2xl">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <main className="flex-1 p-6 overflow-y-auto flex flex-col gap-6">
          ...
          {loading ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center py-24 text-rose-400 font-semibold gap-3 text-center border border-dashed border-rose-500/20 rounded-3xl bg-rose-500/5">
              ...
              <span>{error}</span>
              <button onClick={fetchData} className="px-4 py-2.5 text-xs bg-[#0F172A] border border-slate-800 text-white rounded-xl hover:bg-slate-800 transition-all mt-2">Coba Lagi</button>
            </div>
          ) : filteredProducts.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center py-24 text-slate-500 border border-dashed border-slate-800 rounded-3xl">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {filteredProducts.map(product => {
                ...
                return (
                  <div 
                    key={product.id}
                    className="bg-[#0F172A] border border-slate-800/80 rounded-2xl p-4 flex flex-col justify-between shadow-md hover:-translate-y-1 hover:border-[#7C3AED]/30 hover:shadow-lg hover:shadow-[#7C3AED]/5 transition-all duration-200 cubic-bezier(0.4, 0, 0.2, 1) group relative overflow-hidden"
                  >
                    <div>
                      ...
                      <span className="text-[10px] text-violet-400 font-mono tracking-tight block">{product.sku}</span>
                      <h3 className="font-bold text-sm text-slate-200 mt-1.5 leading-snug group-hover:text-white transition-colors">{product.name}</h3>
                      {product.category && (
                      ...
                      <button
                        ...
                        className={`h-10 px-4 rounded-xl text-xs font-semibold flex items-center gap-1 shadow-sm transition-all active:scale-95 ${
                          isOutOfStock
                            ...
                            : 'bg-[#7C3AED] hover:bg-[#6D28D9] text-white shadow-[#7C3AED]/10'
                        }`}
                      >
        ...
        <aside className="w-[380px] bg-[#0F172A] border-l border-slate-800 flex flex-col justify-between shadow-2xl relative z-20">
          
          {/* Cart Header */}
          <div className="p-5 border-b border-slate-800 flex justify-between items-center bg-[#020617]/30">
            <div className="flex items-center gap-2">
              <ShoppingBag className="w-5 h-5 text-violet-400" />
              <h2 className="font-bold text-sm">Keranjang Pesanan</h2>
            </div>
            <span className="px-2.5 py-0.5 text-xs font-bold bg-[#7C3AED]/10 border border-[#7C3AED]/20 text-violet-400 rounded-full font-mono">
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-3.5 scrollbar-thin">
            {cartItems.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-slate-500 text-center py-16">
                ...
                <p className="text-xs text-slate-500 mt-1 max-w-[200px]">Pilih menu di sebelah kiri untuk ditambahkan.</p>
              </div>
            ) : (
              cartItems.map(item => (
                <div 
                  key={item.product.id}
                  className="bg-[#020617] border border-slate-800 rounded-2xl p-3.5 flex justify-between gap-3 shadow-inner"
                >
                  <div className="flex-1 min-w-0">
                    <h4 className="font-bold text-xs text-slate-200 leading-normal truncate">{item.product.name}</h4>
                    <span className="text-[10px] text-slate-400 font-mono block mt-0.5">Rp {parseFloat(item.product.sell_price).toLocaleString('id-ID')} / pcs</span>
                  ...
                  <div className="flex flex-col items-end justify-between gap-2.5">
                    ...
                    <button 
                      ...
                      className="text-slate-500 hover:text-rose-400 w-9 h-9 flex items-center justify-center rounded-lg hover:bg-rose-500/10 transition-colors active:scale-95"
                    >
                      <Trash2 className="w-4 h-4" />
                    </button>

                    {/* Qty Adjustment */}
                    <div className="flex items-center bg-[#0F172A] border border-slate-850 rounded-xl overflow-hidden p-0.5">
                      <button
                        onClick={() => updateQuantity(item.product.id, item.quantity - 1)}
                        className="w-9 h-9 flex items-center justify-center hover:bg-white/5 text-slate-400 hover:text-white rounded-lg transition-colors active:scale-95"
                      >
                        <Minus className="w-3.5 h-3.5" />
                      </button>
                      <span className="w-8 text-center text-xs font-bold font-mono text-slate-200">{item.quantity}</span>
                      <button
                        onClick={() => updateQuantity(item.product.id, item.quantity + 1)}
                        className="w-9 h-9 flex items-center justify-center hover:bg-white/5 text-slate-400 hover:text-white rounded-lg transition-colors active:scale-95"
                      >
          ...
          <div className="p-5 border-t border-slate-800 bg-[#020617]/50 space-y-4">
            
            {/* Tipe Order Toggle */}
            <div className="space-y-2">
              <label className="text-[10px] text-slate-450 font-bold uppercase tracking-wider">Tipe Pesanan</label>
              <div className="grid grid-cols-2 gap-2 bg-[#020617] p-1 rounded-2xl border border-slate-800">
                <button
            ...
            {orderType === 'dine_in' && (
              <div className="space-y-2 animate-fadeIn">
                ...
                <input
                  ...
                  className="w-full h-11 px-4 bg-[#020617] border border-slate-800 rounded-xl text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 transition-all text-xs"
                />
              </div>
            )}
      ...
      {createdDraft && (
        <div className="fixed inset-0 bg-black/85 backdrop-blur-md flex items-center justify-center p-4 z-50 animate-fadeIn">
          <div className="bg-[#0F172A] border border-slate-800 rounded-3xl w-full max-w-md p-8 text-center shadow-2xl space-y-6">
            ...
            <div className="flex flex-col items-center gap-2">
              ...
              <p className="text-xs text-slate-400">Silakan berikan nomor antrean berikut kepada kasir.</p>
            </div>
            ...
            <div className="bg-white/5 border border-slate-800/80 rounded-2xl p-4 text-left text-xs space-y-2.5">
              ...
              {createdDraft.order_type === 'dine_in' && (
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-slate-200">{createdDraft.table_number}</span>
                </div>
              )}
              <div className="flex justify-between">
              ...
              <div className="flex justify-between border-t border-slate-800 pt-2.5 font-bold text-sm">
                <span className="text-slate-300">Estimasi Subtotal:</span>
            ...
            <button
              ...
              className="w-full h-12 bg-[#7C3AED] hover:bg-[#6D28D9] text-white font-bold text-xs rounded-xl shadow-lg shadow-[#7C3AED]/15 transition-all active:scale-[0.98]"
            >
